...
class Dealer:
  ...
  def __init__(self, small_blind_amount=None, initial_stack=None, ante=None, summary_file=None):
    ...
    self.game_summarizer = GameSummarizer(summary_file)
    self.table = Table()
  ...
  def __publish_messages(self, msgs):
    ...
    self.game_summarizer.summarize_messages(msgs)
...
class GameSummarizer(object):
    ...
    def summarize_messages(self, raw_messages):
        for uuid, message in raw_messages:
            content = message["message"]
            message_type = content["message_type"]
            if message_type == MessageBuilder.ROUND_START_MESSAGE:
                self.round_hole_cards[uuid] = content['hole_card']
            self.summarize(message)

    def summarize(self, message):
        content = message["message"]
        message_type = content["message_type"]
        if MessageBuilder.GAME_START_MESSAGE == message_type:
            self.summarize_game_start(content)
        if MessageBuilder.ROUND_START_MESSAGE == message_type:
            self.summarize_round_start(content)
        if MessageBuilder.ROUND_RESULT_MESSAGE == message_type:
            self.summarize_round_result(content)
        if MessageBuilder.GAME_RESULT_MESSAGE == message_type:
            self.summarize_game_result(content)

    def summarize_game_start(self, message):
        self.game['rule'] = message["game_information"]["rule"]
        for seat in message["game_information"]["seats"]:
            self.game_start_seats[seat['uuid']] = seat
            self.max_participating_round[seat['uuid']] = 0

    def summarize_round_start(self, message):
        for seat in message["seats"]:
            self.round_start_seats[seat['uuid']] = seat
            if seat['state'] == 'participating':
                self.max_participating_round[seat['uuid']] = message['round_count']

    def summarize_round_result(self, message):
        round_info = copy.deepcopy(message)
        for seat in round_info['round_state']['seats']:
            uuid = seat['uuid']
            seat['start_state'] = self.round_start_seats[uuid]['state']
            seat['start_stack'] = self.round_start_seats[uuid]['stack']
            seat['hole_card'] = self.round_hole_cards[uuid]
        for seat in round_info['hand_info']:
            uuid = seat['uuid']
            seat['hand']['hole_card'] = self.round_hole_cards[uuid]
        self.rounds.append(round_info)

    def summarize_game_result(self, message):
        seats = copy.deepcopy(message["game_information"]["seats"])
        for seat in seats:
            uuid = seat['uuid']
            seat['start_state'] = self.game_start_seats[uuid]['state']
            seat['start_stack'] = self.game_start_seats[uuid]['stack']
            seat['max_participating_round'] = self.max_participating_round[uuid]
